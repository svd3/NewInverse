...
def getLoss(theta, coords, actions, agent, batch_size):
    ...
    for i in range(batch_size):
        ...
        for a in true_actions:
            ...
            logPr = logPr -((a.view(-1) - action)**2).sum()/2
...
for pt in range(20):
    ...
    theta = nn.Parameter(theta0.data.clone() + torch.randn(3)*0.08)
    losses = deque(maxlen=100)
    optT = torch.optim.Adam([theta], lr=1e-3)
    prev_loss = 100000
    for ep in range(750):
        loss = getLoss(theta, coords, actions, agent, 200)
        losses.append(loss.data)
        optT.zero_grad()
        loss.backward()
        optT.step()
        print(pt, ep, np.round(loss.data.item(), 6), np.mean(losses), loss0.data)
